...
function addon:CreateOptionsTable()
    return {
        ...
        args = {
            ...
            actionbars = (function()
                if type(addon.BuildActionBarsOptions) == 'function' then
                    return addon.BuildActionBarsOptions()
                else
                    -- Temporary placeholder so the menu node is always visible if the module failed to load
                    return { type='group', name='Action Bars', order=1, args = {
                        _ab_missing = { type='description', order=0, name = function()
                            local ab = (addon and addon.__ab_loaded) and "true" or "false"
                            local bt = type(addon and addon.BuildActionBarsOptions)
                            return string.format("Action Bars module not initialised yet.\n__ab_loaded = %s\nBuildActionBarsOptions type = %s\nIf this persists after /reload, please let the developer know.", ab, bt)
                        end }
                    }}
                end
            end)(),
